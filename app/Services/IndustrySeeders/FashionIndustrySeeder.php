...
final class FashionIndustrySeeder
{
    ...
    {
        // Preset: fashion → primary #111827, secondary #000000 (B-3)
    ...
    {
        $mens = $this->firstOrCreateCategory($tenant, "Men's Fashion");
        $womens = $this->firstOrCreateCategory($tenant, "Women's Fashion");
        $this->firstOrCreateCategory($tenant, 'Accessories');
        // Sub-categories for demo depth
        $this->firstOrCreateCategory($tenant, 'Shirts', $mens->id);
        $this->firstOrCreateCategory($tenant, 'Dresses', $womens->id);
    }
    ...
    private function seedAttributes(Tenant $tenant): void
    {
        ...
        $this->firstOrCreateAttribute($tenant, [
            ...
            'is_variant_defining' => true,
            'sort_order' => 2,
        ], ['Black', 'White', 'Red', 'Blue', 'Green', 'Beige']);
    ...
    private function seedHomepage(Tenant $tenant): void
    {
        ...
        $this->firstOrCreateHomepageSection($tenant, 'product_grid', 'New Arrivals', 2);
    }
