        ...
        @if ($this->hasActiveFilters())
            ...
            <div class="flex flex-wrap items-center gap-2 mb-5">
                @foreach ($brandIds as $id)
                    ...
                    @if ($label)
                        <button
                            wire:click="$set('brandIds', {{ json_encode(array_values(array_diff($brandIds, [$id]))) }})"
                            type="button"
        ...
        <div class="flex flex-col lg:flex-row gap-8">
            ...
            <div class="flex-1 min-w-0 w-full">
                @if ($result['products']->isEmpty())
                    ...
                    @endif
                @else
                    <div class="grid w-full justify-start justify-items-stretch {{ \App\Support\IndustryConfig::currentGet('ui.grid_class', 'grid-cols-2 md:grid-cols-3 lg:grid-cols-4') }} gap-x-4 gap-y-8 sm:gap-x-6"
                        wire:loading.remove
                        ...
                        @foreach ($cards as $card)
                            <x-dynamic-component :component="\App\Support\IndustryConfig::currentGet(
                                'ui.card_component',
                                'storefront.product-cards.default',
                            )" :card="$card" />
                        @endforeach
                    </div>
                    ...
                    <div wire:loading
                        wire:target="sort,inStockOnly,emiOnly,warrantyOnly,onSaleOnly,newArrivalOnly,officialOnly,priceMin,priceMax,brandIds,toggleAttr,clearFilters"
                        class="grid w-full justify-start justify-items-stretch {{ \App\Support\IndustryConfig::currentGet('ui.grid_class', 'grid-cols-2 md:grid-cols-3 lg:grid-cols-4') }} gap-x-4 gap-y-8 sm:gap-x-6">
                        @for ($i = 0; $i < 6; $i++)
                            <div>
