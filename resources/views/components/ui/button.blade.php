@props(['variant' => 'primary', 'size' => 'md', 'type' => 'button', 'loadingTarget' => null])
@php
    $variants = [
        'primary' =>
...
@endphp
<button type="{{ $type }}"
    {{ $attributes->merge(['class' => 'inline-flex items-center justify-center gap-2 rounded-xl font-medium transition disabled:opacity-50 disabled:cursor-not-allowed outline-none ' . ($variants[$variant] ?? $variants['primary']) . ' ' . ($sizes[$size] ?? $sizes['md'])]) }}
