...
<nav class="lg:hidden fixed bottom-0 inset-x-0 z-50 bg-white/95 dark:bg-gray-950/95 backdrop-blur border-t border-gray-200 dark:border-gray-800"
    style="padding-bottom: max(0.375rem, var(--safe-bottom))" aria-label="Mobile primary">
    <div class="flex items-stretch px-1">
        <a href="{{ route('storefront.home') }}"
            class="flex flex-col items-center justify-center gap-1 flex-1 py-2 text-[11px] font-medium {{ request()->routeIs('storefront.home') ? 'text-[var(--brand)]' : 'text-gray-500 dark:text-gray-400' }}">
        ...
        <a href="{{ route('storefront.wishlist') }}"
            class="relative flex flex-col items-center justify-center gap-1 flex-1 py-2 text-[11px] font-medium {{ request()->routeIs('storefront.wishlist') ? 'text-[var(--brand)]' : 'text-gray-500 dark:text-gray-400' }}">
            <span class="relative">
        ...
        <a href="{{ auth('customer')->check() ? route('storefront.account.dashboard') : route('storefront.login') }}"
            class="flex flex-col items-center justify-center gap-1 flex-1 py-2 text-[11px] font-medium {{ request()->routeIs('storefront.account.*') ? 'text-[var(--brand)]' : 'text-gray-500 dark:text-gray-400' }}">
