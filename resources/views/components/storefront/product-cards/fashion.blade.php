...
<div class="group relative flex flex-col overflow-hidden rounded-xl bg-white border border-gray-100 transition duration-200 hover:shadow-md hover:shadow-gray-900/[0.06] dark:border-gray-800 dark:bg-gray-900">
    ...
    >
        <a href="{{ $card['url'] }}" class="absolute inset-0 z-0 focus:outline-none focus-visible:ring-2 focus-visible:ring-[var(--brand)] focus-visible:ring-inset" aria-label="{{ $card['name'] }}">
            @if ($primary)
                ...
                @endforeach
                @if (empty($gallery))
                    <img src="{{ $primary }}" alt="{{ $alt }}" width="400" height="533" loading="lazy" decoding="async" class="absolute inset-0 h-full w-full object-cover object-center {{ $outOfStock ? 'opacity-60 grayscale' : '' }}">
                @endif
            @else
        ...
        @if ($badge)
            <div class="pointer-events-none absolute left-2 top-2 z-[2]">
                <span class="inline-flex items-center rounded-[6px] px-2 py-1 text-[10px] font-bold leading-none tracking-wide shadow-sm {{ $badgeClasses }}">
                    {{ $badge }}
                </span>
            </div>
        @elseif ($discount)
            {{-- already covered by badge branch --}}
        @endif
        ...
        <a href="{{ $card['url'] }}" aria-label="Quick view {{ $card['name'] }}"
            ...
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.75" stroke="currentColor" class="h-3.5 w-3.5 mr-1" aria-hidden="true">
        ...
        @if ($hasGallery)
            <div class="pointer-events-auto absolute bottom-2 left-1/2 z-[2] flex -translate-x-1/2 items-center gap-1.5 rounded-full bg-black/20 px-2 py-1.5 backdrop-blur-sm md:bg-black/15">
                @foreach ($gallery as $idx => $g)
            ...
            {{-- Hide dots counter overlap: when counter is shown, dots remain centered; counter is at right-2 separate --}}
        @endif
    ...
    <div class="flex flex-1 flex-col p-3 pt-3 gap-1">
        ...
        @if (($reviews ?? 0) > 0 && $rating !== null)
            <div class="flex items-center gap-1 text-xs">
                ...
                <span class="font-medium text-gray-800 dark:text-gray-200">{{ number_format((float) $rating, 1) }}</span>
        ...
        <div class="flex items-baseline gap-2 flex-wrap">
            @if ($variant)
                <span class="text-[15px] font-bold leading-none text-gray-900 dark:text-gray-50">{{ money_without_trailing_zeros((int) $variant['price']) }}</span>
                @if ($variant['compare_at_price'] && $variant['compare_at_price'] > $variant['price'])
                    <span class="text-xs font-normal text-gray-400 line-through">{{ money_without_trailing_zeros((int) $variant['compare_at_price']) }}</span>
                    @if ($discount)
            ...
            <div class="flex items-center gap-1.5">
                @if (!empty($swatches))
                    ...
                    @if ($swatchesOverflow > 0)
                        <span class="text-xs font-medium text-gray-600 dark:text-gray-400">+{{ $swatchesOverflow }}</span>
                    @endif
                @else
            ...
            <div class="shrink-0">
                @if ($cta && $cta['type'] === 'add_to_cart')
                    <button type="button" @click="$store.cart.add({{ $cta['variant_id'] }})"
                        ...
                        <span x-show="$store.cart.pending[{{ $cta['variant_id'] }}]" x-cloak class="flex items-center justify-center">
                ...
                @elseif ($cta && $cta['type'] === 'select_options')
                    <x-storefront.variant-modal :card="$card">
                        <x-slot:trigger>
                            <button type="button" @click="open = true" aria-label="{{ __('Add to Cart') }} {{ $card['name'] ?? '' }}"
                                class="inline-flex h-11 w-11 items-center justify-center rounded-xl bg-[var(--brand)] text-white shadow-sm transition hover:brightness-110 hover:shadow-md focus:outline-none focus-visible:ring-2 focus-visible:ring-[var(--brand)] focus-visible:ring-offset-2 dark:focus-visible:ring-offset-gray-900">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.9" stroke="currentColor" class="h-5 w-5" aria-hidden="true">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M15.75 10.5V6a3.75 3.75 0 10-7.5 0v4.5m11.356-1.993l1.263 9.465A2.25 2.25 0 0119.245 21H4.755a2.25 2.25 0 01-2.243-2.532l1.263-9.465A1.5 1.5 0 014.26 7.5h15.48a1.5 1.5 0 011.486 1.507z" />
                                </svg>
                            </button>
