...
<div
    class="group relative flex flex-col overflow-hidden rounded-2xl border border-gray-200 bg-white transition duration-200 hover:shadow-md hover:shadow-gray-900/[0.04] dark:border-gray-800 dark:bg-gray-900">
    ...
    <div x-data="{
        ...
        <a href="{{ $card['url'] }}"
            class="absolute inset-0 z-0 flex items-center justify-center p-2 focus:outline-none focus-visible:ring-2 focus-visible:ring-[var(--brand)] focus-visible:ring-inset"
            aria-label="{{ $card['name'] }}">
            @if ($primary && !empty($gallery))
                @foreach ($gallery as $idx => $g)
                    <img src="{{ $g['src'] }}" alt="{{ $g['alt'] }}" width="300" height="300"
                ...
                @endforeach
            @elseif ($primary)
                <img src="{{ $primary }}" alt="{{ $alt }}" width="300" height="300" loading="lazy"
                    decoding="async"
                    class="absolute inset-0 h-full w-full object-contain object-center p-2 scale-[1.18] {{ $outOfStock ? 'opacity-40 grayscale' : '' }}"
                    draggable="false">
            @else
                <div
                    class="absolute inset-0 flex flex-col items-center justify-center gap-1.5 text-gray-400 dark:text-gray-500">
        ...
        <div class="pointer-events-none absolute left-2 top-2 z-[2] flex flex-col items-start gap-1">
            ...
            @if (($variant['is_preorder'] ?? false) && $discount === null)
                <span
                    class="rounded-md bg-amber-500 px-1.5 py-0.5 text-[10px] font-bold leading-none tracking-wide text-white shadow-sm">{{ __('PRE-ORDER') }}</span>
            @endif
        </div>
        ...
        @if ($hasGallery)
            <div
                class="pointer-events-auto absolute bottom-2 left-1/2 z-[2] flex -translate-x-1/2 items-center gap-1 rounded-full bg-black/10 px-2 py-1.5 backdrop-blur-sm dark:bg-white/10">
                @foreach ($gallery as $idx => $g)
                    <button type="button" @click.prevent.stop="goTo({{ $idx }})"
                        ...
                        :class="activeIndex === {{ $idx }} ? 'bg-gray-800 dark:bg-white w-3' :
                            'bg-gray-400/60 dark:bg-white/60 hover:bg-gray-600/80 dark:hover:bg-white/80'">
                    </button>
                @endforeach
            </div>
    ...
    <div class="flex flex-1 flex-col p-3 pt-2.5 gap-1.5">
        ...
        <div class="min-h-[1.1rem] flex items-center">
            @if (($reviews ?? 0) > 0 && $rating !== null)
                <div class="flex items-center gap-1">
                    ...
                    <span
                        class="text-xs font-semibold text-gray-800 dark:text-gray-200">{{ number_format((float) $rating, 1) }}</span>
        ...
        <div class="flex items-baseline gap-1 flex-wrap min-h-[1.35rem]">
            @if ($variant)
                <span
                    class="text-[13px] sm:text-[15px] font-bold leading-none tracking-tight text-gray-900 dark:text-gray-50 whitespace-nowrap tabular-nums">{{ money_without_trailing_zeros((int) $variant['price']) }}</span>
                @if ($variant['compare_at_price'] && $variant['compare_at_price'] > $variant['price'])
                    <span
                        class="text-[11px] sm:text-xs font-normal leading-none text-gray-400 line-through whitespace-nowrap tabular-nums">{{ money_without_trailing_zeros((int) $variant['compare_at_price']) }}</span>
                    @if ($discount)
                        <span
                            class="text-[11px] sm:text-xs font-bold leading-none text-red-600 whitespace-nowrap">-{{ $discount }}%</span>
                    @endif
                @endif
            @else
            ...
            @if ($cta && $cta['type'] === 'add_to_cart')
                <div x-data="{ variantId: {{ $cta['variant_id'] !== null ? $cta['variant_id'] : 'null' }} }" x-init="if (variantId && $store.cart) $store.cart.pending[variantId] = false">
                    <button type="button"
                        ...
                        class="flex h-10 w-full cursor-pointer items-center justify-center gap-1.5 rounded-xl bg-[var(--brand)] px-3 text-xs font-semibold text-white shadow-sm transition hover:brightness-110 focus:outline-none focus-visible:ring-2 focus-visible:ring-[var(--brand)] focus-visible:ring-offset-2 dark:focus-visible:ring-offset-gray-900 disabled:cursor-not-allowed disabled:opacity-60">
                        <span x-show="!(variantId && $store.cart.pending[variantId])" class="flex items-center gap-1.5">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                stroke-width="1.9" stroke="currentColor" class="h-4 w-4" aria-hidden="true">
