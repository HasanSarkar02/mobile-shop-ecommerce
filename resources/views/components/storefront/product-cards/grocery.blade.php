...
<div class="group relative flex h-full flex-col overflow-hidden rounded-xl border border-gray-200 bg-white transition-all duration-200 hover:shadow-md hover:shadow-gray-900/5 dark:border-gray-800 dark:bg-gray-900">
    <div class="relative">
        <a href="{{ $card['url'] }}" class="block focus:outline-none focus-visible:ring-2 focus-visible:ring-green-600 focus-visible:ring-offset-2 dark:focus-visible:ring-offset-gray-900 rounded-t-xl">
        ...
        <div class="absolute top-2 right-2 z-20 cursor-pointer pointer-events-auto">
            <x-storefront.wishlist-button :id="$card['id']" :wishlisted="$card['wishlisted']" />
        </div>
    </div>

    <div class="flex flex-1 flex-col p-3">
        <a href="{{ $card['url'] }}" class="block focus:outline-none focus-visible:ring-2 focus-visible:ring-green-600 rounded">
    ...
    <div class="px-3 pb-3">
        @if ($variantId && $isPurchasable)
            <button type="button" @click="$store.cart.add({{ $variantId }}, {{ $sellByUnitFloat }})"
                :disabled="$store.cart.pending[{{ $variantId }}]"
                :aria-busy="$store.cart.pending[{{ $variantId }}] ? 'true' : 'false'"
                class="w-full bg-[#16a34a] hover:bg-green-700 text-white font-semibold rounded py-2 flex items-center justify-center gap-2 transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-green-600 disabled:opacity-60 disabled:cursor-not-allowed">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.75" stroke="currentColor" class="w-5 h-5" aria-hidden="true">
                ...
                <span x-show="$store.cart.pending[{{ $variantId }}]" x-cloak class="inline-flex items-center gap-1.5">
