...
12. [~] **Static analysis + lint** (Pint, Larastan per `pint.json`, `phpstan.neon`). Both gates are green as of 2026-08-23: **Pint reports 0 files needing changes** and **Larastan reports 0 errors at level 5**. **Remaining:** the baseline is far larger than previously recorded — a 2026-08-24 recount found `phpstan-baseline.neon` holds **225 ignore blocks covering ~315 suppressed errors**, not "41 lines". The earlier "down from 329 to 41" figure was wrong or measured something else; scoping and retiring these is the real open work here.
...
16. [x] **P5.2 /pre-order discovery.** `GET /pre-order` (`routes/tenant.php:43` `PreorderController.php:10` `Product::published()->whereHas variants preorder`), `index.blade.php`, product-card ETA badge. **Open tail (verified still missing 2026-08-24):** no nav link to `/pre-order` exists in any header component (`desktop-header`, `mobile-menu`, `mobile-header`).
...
48. [ ] **De-phone schema (deprecate, don't drop).** Mark `color/storage_gb/ram_gb/sim_type/region` variant columns (`add_specs_pricing_media_to_product_variants_table.php:13-18`) as deprecated/no-op; add EAV-backed equivalents; **no data migration, no column removal**. `ProductController.php:119-131` reads move to attribute-driven with fallback to deprecated columns. **F.3 audit (2026-08-24) — the display-side dual-read already exists:** `ProductController::show()` merges native columns (color/storage/region only, lines 119-131) and variant-defining EAV attributes (lines 134-148) into one unified `dims` structure for the PDP selection engine; `VariantSignatureService` already exempts phone-native variants from combination-uniqueness via null signatures. Two decisions still owed here: (a) `ram_gb`/`sim_type` are **dead on the storefront** — never read by any controller; confirm they stay deprecated rather than being wired in; (b) **precedence is implicit** — when an EAV attribute coded `color` coexists with a populated native column, native wins silently (`$dims[$code] ??=`); make this explicit (document or resolve) during this item.
...
- **Next — small polish batch:** retire the baseline suppressions (#12 — real scope ~315 errors, needs scoping first) · rotate the exposed SMTP password (#13) · add real coverage for customer login + `/account` (currently **zero** tests — the only file that touched those routes was a scratch debug test that asserted nothing and has been deleted; owner reports manual verification passes) · pre-order nav link (open tail of #16).
